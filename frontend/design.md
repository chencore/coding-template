...
- **人文导师团回应卡**（renwen-mentors 新增）：沿用导师回应卡样式，差异：元信息行为引荐语「{导师名} 请来了 {人物名}」；回应正文下方加 10px 淡墨「出处 · {篇名}」（篇名来自后端出处库，不由前端拼）；卡右下方 11px 淡墨「今天还能再请 N 次」。召唤页人物选择复用设置页选项卡（「让导师代选」为默认选中项）；429 用 SnackBar「今天已请过三次了，明天再来。」；503/不可达用内联可重试提示，已填困惑不丢
